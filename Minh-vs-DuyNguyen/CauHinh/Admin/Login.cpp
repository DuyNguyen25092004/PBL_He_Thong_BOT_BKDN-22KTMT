...
class login{
    ...
    void sdt_mk_corrected(){
        string s_sdt,s_mk;
        docfile.open("C:/Users/MSI PC/Desktop/pbl_adjust/Minh-vs-DuyNguyen/CauHinh/Admin/txt/AccountAdmin.txt",ios::in);
        if(docfile.is_open()){
            while (docfile>>s_sdt){
                if(s_sdt==sdt){
                    docfile>>s_mk;
    ...
    void start_save_history(){
        time=time_in_life();
        if(k==1){
        docfile.open("C:/Users/MSI PC/Desktop/pbl_adjust/Minh-vs-DuyNguyen/CauHinh/Admin/txt/LoginHistory.txt",ios::app);
        if(docfile.is_open()){
